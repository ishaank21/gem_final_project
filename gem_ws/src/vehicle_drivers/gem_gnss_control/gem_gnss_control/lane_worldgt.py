...
class WorldGT:
    def __init__(self, world: str, resources_dir: str,
                 x: float = None, y: float = None, yaw: float = None) -> None:
        data = {}

        with open(os.path.join(resources_dir, f"{world}_centerline.csv")) as f:
            reader = csv.reader(f)
            map = {}
        ...
        with open(os.path.join(resources_dir, f"{world}_parameters.yaml")) as f:
    ...
    def get_metrics(self, x: float, y: float, yaw: float):
        pt = np.array([x, y])
